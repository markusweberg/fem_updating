...
def get_MAC_MMI(freqs_est, modes_est, freqs_num, modes_num, filtering=False,
    ...
    results_MAC : dict
        Dictionary with the best MAC values (sorted) for each estimated
        mode shape evaluated.
    results_MMI : dict
        Dictionary with the best MMI values (sorted) for each estimated
        mode shape evaluated.
    """
    ...
    for k in range(cols_est):
        ...
        results_MMI.update({mode_est: SI_sort_MMI_dsc})
        results_MAC.update({mode_est: SI_sort_MAC_dsc})
