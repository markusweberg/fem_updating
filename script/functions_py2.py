...
def get_modeshapes(odb, node_set_name_1, node_set_name_2, save=False, name='test', folder='test'):
    ...
    for i in range(number_of_frames):
        ...
        # Set
        node_displacement_1 = displacement.getSubset(region=node_set_1)
        node_displacement_2 = displacement.getSubset(region=node_set_2)

        # Mode shape vector
        phi = np.zeros(channels, dtype=float)

        # Looping through all nodes in the node set
        for idx, node in enumerate(node_displacement_1.values):
        ...
        for idx, node in enumerate(node_displacement_2.values):
            ...
            # Save file to column
            # Save to matrix
            phi[(3*idx + len(node_set_1.nodes[0])*3) + 0] = X
            phi[(3*idx + len(node_set_1.nodes[0])*3) + 1] = Y
            phi[(3*idx + len(node_set_1.nodes[0])*3) + 2] = Z
